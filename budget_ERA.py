...
#########################################################            
####### file 2
filenm='eddydiffradcon_'+casenm+'_Allcomps.txt'
...
for it in range(0,nt):
    for iz in range(0,nz):                 
        q1c[it,iz]=(eddyvar[12,it,iz]+eddyvar[13,it,iz])*scq1      \
                    +  eddyvar[14,it,iz]*scq2                   \
                    + (eddyvar[15,it,iz]+eddyvar[16,it,iz])*scq3  
        q2c[it,iz]=( eddyvar[12,it,iz]+eddyvar[13,it,iz]      \
                    + (eddyvar[14,it,iz]+eddyvar[15,it,iz]*2.5e10/2.834e10) )*scq4 
q1c[it,0]=0.0
q2c[it,0]=0.0
...
for iz in range(0,nz):    
    for iv in range(0,5):
        tmp3=0.0
        for it in range(starid,nt-1):
            tmp3=tmp3+eddyvar[iv+12,it,iz]/(nt-starid-1.)
...
ax0.set_title('Case '+casenm+' Water content profiles'+ r' $g$ $kg^{-1}$',fontsize=20)
ylabs='Height'+r' $km$'
ax0.set_ylabel(ylabs,fontsize=20)
#ax0.set_xticks(([0,0.02,0.04,0.06,0.08]))
ax0.legend()
#
plt.show()                     
plt.savefig(dirpic+casenm+'_qaqbqcqr_profiles2_2.pdf')          
#plt.show()
plt.close() 
##############################
# Q1 and Q2
...
#####
plt.show()                     
plt.savefig(dirpic+casenm+'_Q1Q2_profiles2_2.pdf')          
#plt.show()
plt.close() 
...
ax5.axis('off')
plt.show()                     
plt.savefig(dirpic+casenm+'_Q1Q2_profiles_Composition2_2.pdf')          
#plt.show()
plt.close() 
