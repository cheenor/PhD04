      PARAMETER (in=121, kn=33)
      PARAMETER (im=121, km=52, NZM=km-1)
      PARAMETER (it=121)
...
!      dimension tme1flux(it,km),tme2flux(it,km),tmeflux(it,km)      
      dimension qabcr(13,it,km),tmpwork(it,km)
...
!     tmc(it,:),tme(it,:),tmd(it,:),tms(it,:),tmf(it,:)
      dimension obsq12(2,it,km)
!     q1    q2
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      character path*100,fold*20,casenm*10,obsq12nm*20
      
      casenm='ETP2D1'
      obsq12nm='ETP06'
!!!!!!!!!!! file 1
      fold='runC01'
      path='/home/jhchen/jhchen/CRM/China/TP/'      
      open(30,file=trim(path)//trim(fold)//'/eddydiffrad_'//trim(casenm)//
     * '_All.txt' )
      open(31,file=trim(path)//trim(fold)//'/'//trim(casenm)//
     * 'regrid_eddydiffrad_All.txt' )
      open(40,file=trim(path)//trim(fold)//'/'//trim(casenm)//
     * '_All.txt' )
      open(41,file=trim(path)//trim(fold)//'/'//trim(casenm)//
     * 'regrid_qabcr_All.txt' )
      open(50,file=trim(path)//trim(fold)//'/postdata/'//trim(casenm)//
     * '_micro_202_6hour.txt' )
      open(51,file=trim(path)//trim(fold)//'/postdata/'//trim(casenm)//
     * '_micro_regrid_6hour.txt' )
      open(60,file=trim(path)//trim(fold)//'/'//trim(obsq12nm)//'.99'
      open(61,file=trim(path)//trim(fold)//'/'//trim(obsq12nm)//'_regrid.99'
      open(70,file=trim(path)//trim(fold)//'/gridded_height_33levels.txt'
...
cc interpolate into regular grid:
    do mt=1,it
      read(30,99)eddydiffrad(1,mt,:),eddydiffrad(2,mt,:),eddydiffrad(3,mt,:),
...
c     to(i,k)=to(i,k)-uo(i,k)
    ...
    enddo  
    do mt=1,it
      write(31,99)(eddydiffrad(1,mt,k),k=1,nzg),(eddydiffrad(2,mt,k),k=1,nzg)
...
c     to(i,k)=to(i,k)-uo(i,k)
    ...
    enddo 
    do mt=1,it
      write(41,99)(bugetqc(1,mt,k),k=1,nzg),(bugetqc(2,mt,k),k=1,nzg)
