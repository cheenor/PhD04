...
!        READ(40,102)ITTSP,PRESSSS,GZSS,UUSS,VVSS,     !!! 4 = SURFACE
!     +       WWSS,TEMPSS, THESS,VAPSS,VQ1,VQ2
...
!--------------READ THE Q1 AND Q2 WHCIH IS WRITTEN BY CHENJINGHUA 
        DO IK=1,NPIN0
          READ(90,906)ITMID,(XYD(IK,KK),KK=1,18)
C PRINT*,CELORD
C STOP
        ENDDO
...
!     +'18 TCHT(K/DAY)','19 ORI_OMEGA(PA/S)'
      ...
      DO IT=1,NTDAT
        ...
        Q2T=0.
        DO IK=1,17
          READ(20,906)ITTS,(XYD(IT,IK,KK),KK=1,18)
C PRINT*,CELORD(IT)
        ...
        Q1Q2(IT,1)=Q1T
        Q1Q2(IT,2)=Q2T
        READ(30,907)ITTS,(XYS(IT,KK),KK=1,10) !LH(W/M^2)   SH(W/M^2)       PRATE      CPRATE  DLR(W/M^2)  ULR(W/M^2)
      ENDDO
!---------------OUTPUT-----------------------------------------
      ILEN=LEN_TRIM(FRAW)
      ...
      DO IT=1,NTDAT
        WRITE(40,1014)IT*1.0,Q1Q2(IT,1),Q1Q2(IT,2),XYS(IT,3)
